...
class HotelService:
    ...
    def update_hotel_amenities(self, hotel_id, amenities):
        try:
            if not isinstance(amenities, list):
                return {'error': 'Amenities must be a list', 'status_code': 400}
            ...
            if result.modified_count > 0:
                # Publish message to RabbitMQ
                publish_message('hotel.amenities_updated', {
                    '_id': hotel_id,
                    'amenities': amenities
                })
            ...
            current_app.logger.error(f"Error updating hotel amenities: {str(e)}")
            return {'error': str(e)}
    
    def add_hotel_image(self, hotel_id, image_url):
        try:
            result = self.db.hotels.update_one(
                {'_id': ObjectId(hotel_id)},
                {
            ...
            if result.modified_count > 0:
                # Publish message to RabbitMQ
                publish_message('hotel.image_added', {
                    '_id': hotel_id,
                    'image_url': image_url
                })
            ...
            current_app.logger.error(f"Error adding hotel image: {str(e)}")
            return {'error': str(e)}
    
    def update_hotel_contact(self, hotel_id, contact_data):
        try:
            ...
            if result.modified_count > 0:
                # Publish message to RabbitMQ
                publish_message('hotel.contact_updated', {
                    '_id': hotel_id,
                    'contact': contact_data
                })
            ...
            current_app.logger.error(f"Error updating hotel contact: {str(e)}")
            return {'error': str(e)}
